...
import com.crypto.task2.report.DataBean;
import com.crypto.task2.report.ReportUnits;
...
public class ReporterData {
    ...
    public List<DataBean> testResults(IResultMap map, int status) {
        ...
        for (ITestResult result : sortByTime(map.getAllResults())) {
            DataBean data = new DataBean();
            List<String> listOutputDataList = new ArrayList<String>();
            Object outputData=result.getAttribute("data");
            
            listOutputDataList.add((String)outputData);
            ...
            data.setOutput((String)outputData);//Reporter.getOutput(result) listOutputDataList
